...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps) {
  return (
    <Dialog.Root>
      <Dialog.Trigger className='rounded-md text-left flex flex-col bg-slate-800 px-5 pt-5 gap-2 overflow-hidden relative outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
        <span className='text-xs font-medium text-slate-300'>
          {formatDistanceToNow(note.date, { locale: ptBR, addSuffix: true })}
        </span>
        <p className='text-sm leading-6 text-slate-400 h-full overflow-scroll'>
          {note.content}
        </p>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className='fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 rounded-md flex flex-col outline-none overflow-hidden'>
          ...
          <div className='flex flex-1 flex-col gap-3 px-5 pt-5 h-full'>
            ...
            <p className='text-md leading-6 text-slate-400 overflow-scroll md:h-[calc(100%-96px)] pb-4'>
              {note.content}
            </p>
          </div>
